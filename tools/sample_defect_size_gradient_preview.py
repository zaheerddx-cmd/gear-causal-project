...
def make_board(items, out_path: Path, cols=2, cell=320):
    if not items:
        return
...
def main():
    ...
    for name, lo, hi in bins:
        pool = [x for x in items if (x["ratio"] >= lo and x["ratio"] < hi)]
        count = len(pool)
        picks = random.sample(pool, min(args.per_bin, count)) if count > 0 else []
        make_board(picks, out_dir / f"{name}.jpg", cols=2, cell=320)
        lines.append(f"{name}: range=[{lo:.4f}, {hi:.4f}) count={count} picked={len(picks)}")
        for p in picks:
