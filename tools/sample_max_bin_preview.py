...
def main():
    ...
    if not items:
        raise RuntimeError(f"没有找到面积占比 >= {args.thr} 的样本")

    picks = random.sample(items, min(args.n, len(items)))
    out_dir = root / "size_maxbin_preview"
    out_dir.mkdir(parents=True, exist_ok=True)
    make_board(picks, out_dir / f"random_ge_{args.thr:.3f}.jpg", cols=2, cell=320)

    lines = [f"thr={args.thr}", f"total_ge_thr={len(items)}", f"picked={len(picks)}"]
    for x in sorted(picks, key=lambda z: z["ratio"], reverse=True):
        lines.append(f'{x["stem"]} ratio={x["ratio"]:.6f}')
